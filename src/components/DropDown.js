...
export const DropDown = ({ children, className,options, ...rest }) => {
    ...
    const combined = twMerge(" border border-solid border-[#D9DEE2] text-gray-500 rounded px-4 py-4 flex justify-center items-center text-sm hover:bg-gray-200 ",className)
  ...
  <Select.Root {...rest} >
    ...
    <Select.Portal className="">
      <Select.Content className="bg-white border-[1px] p-1 rounded z-50">
...
const SelectItem = React.forwardRef(
  ({ children, className, ...props }, forwardedRef) => {
    return (
      <Select.Item
        className="hover:bg-[#6D5DE7] hover:text-white text-sm p-1  m-1 flex items-center rounded py-1.5 pl-3 "
        {...props}
        ref={forwardedRef}
      >
